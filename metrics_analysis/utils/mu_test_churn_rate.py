...
for repo_id in repo_id_list:
    ...
    stat, p = mannwhitneyu(test_churn_rate_list, production_churn_rate_list)
    print('Statistics=%.3f, p=%.10f' % (stat, p))

    if p <= 0.05:
        d, res = cliffsDelta.cliffsDelta(test_churn_rate_list, production_churn_rate_list)
        print(d)
        print(res)

stat, p = mannwhitneyu(total_test_churn_rate, total_production_churn_rate)
print('total Statistics=%.3f, p=%.10f' % (stat, p))

if p <= 0.05:
    d, res = cliffsDelta.cliffsDelta(total_test_churn_rate, total_production_churn_rate)
    print(d)
    print(res)
